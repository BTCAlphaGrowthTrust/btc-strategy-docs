...
description: The gated Demeter program API and the keyless marketing preview — manifest, basket, deploys, performance, profile and simulate.
...
Demeter exposes **two surfaces**: a **gated product API** for entitled subscribers, and a **keyless
...
Every gated endpoint requires an `X-API-Key` header **and** the **`demeter` entitlement** on that
...
`X-API-Key` alone is not enough — the key must carry the `demeter` entitlement. A signal-feed key
